...
( function() {
  ...
  plug.rtf = {
    ...
    merge: function( obj1, obj2 ) {
      ...
      self.forEach( self.keys( copy2 ), function( key ) {
        if ( typeof copy2[ key ] === 'object' && typeof copy1[ key ] === 'object' ) {
          copy1[ key ] = self.merge( copy1[ key ], copy2[ key ] );
        } else {
          copy1[ key ] = copy2[ key ];
        }
      } );
  ...
  function removeSuperfluousStyles( element ) {
    ...
    for ( styleName in styles ) {
      ...
      if ( some( resetStyles, function( val ) {
  ...
  function some(array, fn, thisArg) {
